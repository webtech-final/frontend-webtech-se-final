        ...
        <div class="bg-gray-900 mx-52 rounded-xl">
            ...
            <div style="margin-top: 40px">
                ...
                <div style="margin-top: 10px;">
                    <label for="name error" style="color:red;" v-if="error.name">{{
                        error.name
                    }}</label>
                </div>
            </div>
            <div style="margin-top: 40px">
                ...
                />
                <div style="margin-top: 10px;">
                    <label for="email error" style="color:red;" v-if="error.email">{{
                        error.email
                    }}</label>
                </div>
            </div>
            <div style="margin-top: 40px">
                ...
                />
                <div style="margin-top: 10px;">
                    <label for="password error" style="color:red;" v-if="error.password">{{
                        error.password
                    }}</label>
                </div>
            </div>
            <div style="margin-top: 40px">
                ...
                <div style="margin-top: 10px;">
                    <label
                        for="confirm password error"
                        style="color:red;"
                        v-if="error.confirmPassword"
                        >{{ error.confirmPassword }}</label
                    >
                </div>
            </div>
            <div class="mt-10 pb-10">
                <button
                    class="bg-blue-500 hover:bg-blue-600 text-xl w-96 rounded-xl p-3"
                    @click="signup"
                >
                    SIGN UP
                </button>
...
export default {
    ...
    methods: {
        async signup() {
            ...
            let res;
            try {
                res = await AuthUser.dispatch('register', payload);
            } catch (error) {
                console.log(error);
            }
            if (res.status === 400) {
