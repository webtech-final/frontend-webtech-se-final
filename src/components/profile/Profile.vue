<template>
    <div
        class="card-bg rounded-lg shadow-lg bg-gray-600 w-full flex flex-row flex-wrap p-3 antialiased"
    >
        <div class="w-1/6">
            <img
                v-if="!this.isProfilePicNull()"
                class="h-64 w-64 rounded-lg shadow-lg antialiased text-gray-300"
                v-bind:src="picPath()"
                alt="Profile Picture"
            />
            <img
        ...
        <div class="w-5/6 pl-5 flex flex-row flex-wrap">
            <div class="w-full text-left text-gray-700 font-semibold relative pt-0">
                <div class="text-5xl text-white leading-tight">{{ currentUser.name }}</div>
                <br />
                <div class="text-2xl text-gray-300">
                    ...
                    <br />
                    <div>
                        Point: <b>{{ currentUser.point }}</b>
                    </div>
                </div>
                <div class="text-xl text-gray-300 absolute pt-0 bottom-0 right-0">
                    Joined Date: <b>{{ this.dateFormatter(currentUser.created_at) }}</b>
                </div>
            </div>
        </div>
    </div>
