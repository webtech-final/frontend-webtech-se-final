<template>
    <div class="bg pt-20">
        <div class="relative">
            <div style="display: grid; grid template-columns: 100%" class="absolute top-0 mx-auto w-full -mt-8">
                <div class="h1 bg-yellow-50 mx-52 rounded-lg p-3">
                    <div class="font-serif text-3xl">LEADERBOARD </div>
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y-8 divide-gray-900">
                        <tr class="cursor-pointer p-16 bg-yellow-100 hover:bg-yellow-200" v-for="(info,index) in single" :key="index" v-on:click="clickSingle(info)">
                            ...
                            <td>
                                <div class="py-1 px-8">
                                    <img v-if="!isProfilePicNull(info.user.image)" class="w-12 h-12 rounded-lg shadow-lg" v-bind:src="`${picPath(info.user.image)}`" alt="">
